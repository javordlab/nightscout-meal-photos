...
- [ ] Keep existing async cron sync (`Radial Sync (30m)`) as fallback safety net.
...
- [ ] Ensure Telegram media messages always produce a durable photo URL for log rows.
- [ ] If image tool fails (e.g., `Unsupported media type: document`), route through photo pipeline fallback.
- [ ] Fallback policy: use pipeline-created public URL (`iili.io/...`) instead of Telegram message URL when Telegram payload is `document`.
- [ ] Add handoff step: before finalizing Food row, check pending photo queue by message/file prefix and inject `[📷](public_url)`.
- [ ] Guarantee `[📷](...)` link insertion in `health_log.md` before sync when a valid photo exists.
- [ ] Add validation gate: Food entry came from image message but `photoUrls.length===0` => warning/error.
- [ ] Add regression test for Telegram image-as-document payloads.

### 3) Verification / observability
- [ ] Add watchdog check: recent Telegram Food image message IDs must map to Notion entries with `Photo` URL.
...
- [ ] Implement normalized content classifier: `TEXT`, `PHOTO`, `PHOTO_TEXT`, `IMAGE_DOCUMENT`, `DOCUMENT_NON_IMAGE`, `VOICE`, `VIDEO`, `UNSUPPORTED_MEDIA`, `COMPOSITE_EVENT`.
- [ ] Detect `IMAGE_DOCUMENT` via `document.mime_type` (`image/*`) and/or file extension; do not treat as generic document.
...
- [ ] If public upload fails, preserve local media reference (`filePrefix`, local path, message_id, timestamp) and queue retry.
- [ ] Mark state explicitly as `upload_failed_pending_retry` (fail-open for capture, fail-closed for completion claim).
- [ ] For food-from-photo rows, insert pending marker (`[📷](pending:...)`) or hold completion per strict mode.
- [ ] Block/flag complete Notion sync for food-photo rows lacking resolved public photo URL.
- [ ] Retry uploads with backoff schedule (e.g., 1m, 5m, 15m, 1h), then alert after max retries.
- [ ] On later upload success, patch `health_log.md` row with real URL and re-upsert Notion/Nightscout.
