...
// Returns midnight PDT/PST for a given date offset from today (0 = today, -1 = yesterday)
function pdtMidnight(dayOffset = 0) {
  ...
  // Parse as LA midnight → UTC epoch
  return new Date(`${dateStr}T00:00:00-${tzOffset()}`).getTime();
}

function tzOffset() {
  // Returns current PDT/PST offset string e.g. "07:00" or "08:00"
  const now = new Date();
  const utcH = now.getUTCHours();
  const laH = Number(new Intl.DateTimeFormat('en-US', {
...
async function main() {
  try {
    ...
    // Previous calendar day: midnight-to-midnight PDT
    ...
    console.log(`  -> glucose_24h.json updated (${dataPrevDay.length} readings, prev day PDT).`);
    ...
    console.log(`  -> glucose_14d.json updated (${data14d.length} readings, 14d PDT).`);
