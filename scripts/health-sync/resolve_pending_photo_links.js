...
function loadPendingPhotos() {
  ...
  try {
    ...
    return raw
      .filter(item => item && /^https?:\/\//i.test(String(item.photoUrl || '')))
      .map(item => ({
        ...item,
...
function findPendingPhotoUrl(row, pendingPhotos) {
  if (!/\[[^\]]*\]\(\s*pending\s*\)/i.test(row.entryText)) return null;
...
function applyReplacement(line, url) {
  // If same URL already exists in the row, drop pending marker entirely.
  if (line.includes(`](${url})`)) {
    return line.replace(/\s*\[[^\]]*\]\(\s*pending\s*\)/ig, '');
  }
  // Otherwise replace pending marker with resolved URL.
  return line.replace(/\[[^\]]*\]\(\s*pending\s*\)/ig, `[📷](${url})`);
}
...
async function main() {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (!/\(\s*pending\s*\)/i.test(lines[i])) continue;
    ...
    const next = applyReplacement(lines[i], resolvedUrl);
    if (next !== lines[i]) {
      lines[i] = next;
      updated++;
    }
