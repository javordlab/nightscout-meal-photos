...
### Issue 37: Prediction Model v3 → v4 recalibration — SHIPPED 2026-06-12
**Root Cause:** Model v3 (calibrated 2026-04-02 on n=57) claimed ~87–89% within ±20 mg/dL; measured reality on 145 clean meals (Feb–Jun, no meal-stacking in the 3h peak window) was MAE 20.6, only 57% within ±20, with systematic over-prediction (bias −6.4). Three structural errors: (1) the 31–50g carb factor ×1.2 was nearly 50% too high (implied ~0.8, and v3's factor curve wasn't even monotonic); (2) preBG was carried 1:1 into the prediction while reality damps it ~35% (over-predicts ~15 at preBG 130+, under-predicts ~19 below 90); (3) time-to-peak ran 20–70 min late for Lunch/Dinner/Snack (actual medians 74/55/55 vs 113/76/126).
**Fix:** Model v4 — factors 2.0/1.2/0.9/0.7, intercepts Breakfast +25 / Lunch −5 / Dinner 0 / Snack 0 / Dessert −10, new damping term `−0.35 × (preBG − 115)`, time-to-peak 87/75/55/60/95, output band widened ±5 → ±10. Holdout-validated (fit on Mar–Apr, tested on unseen May–Jun): MAE 21.8 → 16.1; full set 20.6 → 14.9 (−28%), ±20 57% → 68%, ±30 76% → 90%, bias ≈ 0. Full analysis: `docs/model_v4_calibration_2026-06-12.md`. Updated as a FIVE-copy parity contract: `foodlog-cwd/CLAUDE.md` Step 4, `scripts/calculate_notion_projections.js` (incl. damping in the fallback formula), `AGENTS.md`, workspace `CLAUDE.md`, `WORKFLOW_AUTO.md`, plus the `defaultPhotoPrompt` string in `config.foodlog.json`. Foodlog bridge restarted (targeted kickstart) and projections script verified live (0 failed). Side finding kept out of the model: measured average post-meal-walk effect is only ~2–5 mg/dL vs prediction, not the 20–40 "peak clip" the Coach narrative cited — Coach claims should soften accordingly.

### Issue 38: Daily-report alert storm — one failed 08:55 send produced 1,200+ duplicate Telegram alerts — FIXED 2026-07-02
**Root Cause (four stacked amplifiers on top of one transient network blip):** (1) The 08:55 Telegram send had no retry; a VPN-tunnel connect blip (Node `AggregateError`, whose `.message` is empty — logged as a useless `error:""`) killed the run. (2) launchd keeps `last exit code = 1` for 24h, and `probe_launchd_jobs.js` re-reported it as a fresh failure every 5 min, overwriting the real `daily-report` heartbeat. (3) The probe's own `partial` receipt incremented `consecutiveErrors` in heartbeat_wrap on every tick (reached 123). (4) The watchdog's alert fingerprint included that counter, so "changed=true" every 15 min → 1,240 duplicate alert sends. Bonus: `report_watchdog.js` exited 1 even when its fallback successfully delivered the report, and 2026-06-25..28 the `[200,320]` fail-closed stats gate killed the report on a genuine CGM sensor gap.
**Fix:** (a) `net_retry.js` gains `withNetRetry`/`isPreConnectionError`/`describeError` — retries DNS + connect-phase failures (incl. AggregateError) that never reached the server; both report senders now retry sends ~5× with backoff and log real error messages. (b) `daily-report` marked `selfHeartbeat: true` — the probe only checks loaded-ness, never overwrites its heartbeat or re-reports sticky exit codes. (c) heartbeat_wrap counts only `error` toward `consecutiveErrors`. (d) Watchdog fingerprint uses status (stable), not counters; 4h forceResend unchanged. (e) `report_watchdog.js` exits 0 + `warn` receipt when the fallback delivered. (f) Watchdog downgrades a failed `daily-report` to `warning` once `report_status.json` shows today covered. (g) Stats gate is now mirror-freshness-aware: stale MySQL glucose mirror still fails closed (`glucose_mirror_stale`), but a genuine sensor gap with a live mirror ships the report with a "Partial CGM coverage" banner. (h) `checkDailyReport` hardcoded `America/Los_Angeles` → host TZ (machine is Europe/Madrid since mid-2026).
